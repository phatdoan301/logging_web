...
TCP_PORT = 6006
WS_PORT = 8014
base_log_dir = "logs_cabinet_test1"
drive_dir = '17X46IUy_m3NSt84kvEGbRx51CoF94-BE'
...
async def tcp_listener():
    ...
    tcp_sock.listen(5)  # Listen for up to 5 clients at a time
    ...
    while True:
        try:
            client_sock, addr = await loop.sock_accept(tcp_sock)
            print(f"Accepted new connection from {addr}")
            loop.create_task(handle_tcp_client(client_sock, addr, log_file, local_folder_path))
        except Exception as e:
...
async def handle_tcp_client(client_sock, addr, log_file, local_folder_path):
    loop = asyncio.get_event_loop()
    try:
        while True:
...
async def main():
    # Run TCP listener and WebSocket server concurrently
